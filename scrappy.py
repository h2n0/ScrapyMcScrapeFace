...
class scrape_handler:
  ...
  def makeFolder(self,sub):
    self.cd = sub
    if not (os.path.exists(self.pos + "/" + self.cd)):
      os.mkdir(self.pos+"/"+cd, 777)


  #Just saves the image		
  def saveSingleImage(self,d):
    imageData = urllib2.urlopen(d).read()
    dotLocation = d.rfind(".")
    ending = d[dotLocation:dotLocation+4]
    imageFileName = self.pos + "/" + self.cd + "/Image{:04d}{}".format(self.count, ending)
    self.putDataInFile(imageFileName, imageData)
    self.count = self.count + 1

      
  def putDataInFile(self,f, d):
    with open(f, "w") as fl:
      fl.write(d)
  ...
  def lookAtAlbum(self,s):
    ...
    req.add_header('Authorization', 'Client-ID ' + selfclientID)
...
scraper = scrape_handler()
if __name__ == "__main__":
    #lookAtAlbum(sys.argv[1])
    scraper.lookAtAlbum(sys.argv[1])
